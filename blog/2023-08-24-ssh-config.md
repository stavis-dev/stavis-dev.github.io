...
## Ошибка изменения идентификации
...
или если известен только IP адрес:

```bash
ssh-keygen -R IP_адрес_сервера
```

Если подключение было к какому то кастомному порту, отличному от 22го тогда его следует прописать:
