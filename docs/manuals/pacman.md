...
Расшифровка ключей `-Rns`

* -R (Remove): Базовая команда на удаление пакета. Сам по себе этот ключ удалит только указанный пакет, оставив в системе все его зависимости и конфигурационные файлы.
* -n (nosave): Указывает pacman не создавать резервные копии конфигурационных файлов.
* -s (recursive): Удаляет пакет вместе со всеми его зависимостями, при условии, что они не нужны другим программам в системе и не были установлены пользователем вручную. Это главный дворник, который защищает систему от «осиротевших» (orphaned) пакетов.

По умолчанию, если вы изменяли глобальные конфиги программы, pacman сохраняет их с расширением `.pacsave`. Ключ `-n` заставляет его полностью стереть эти файлы. Внимание: настройки в вашей домашней директории (~/.config или ~/.package) этот ключ не трогает, их нужно удалять вручную.
