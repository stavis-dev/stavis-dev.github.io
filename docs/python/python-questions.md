...
draft: false
source_from: "https://github.com/AlexRogalskiy/python_interview_questions/blob/master/questions.md"
...
_Последовательностью_ в Python называется итерабельный объект, который поддерживает эффективный доступ к элементам,
...
К основным встроенным типам последовательностей относятся [list](./types/python-types-list.md), [tuple](./types/python-types-tuple.md), range, [str](./types/python-types-string.md) и bytes.

Последовательности также опционально могут реализовывать методы 

- [count()](./types/python-types-list.md#list-coint), 
- [index()](./types/python-types-list.md#list-index), 
- `__contains__()` и 
...
В третьем Питоне есть один вид строк `str`, который представляет собой _Юникод_.  
Есть тип `bytes`, то есть цепочка _байт_.

### Можно ли изменить отдельный символ внутри строки

Нет, строки **неизменяемы**. Операции _замены_, _форматирования_ и _конкатенации_ возвращают **новую** строку.
...
Кодировать – перевести _Юникод_ в байтовую строку. Вызвать метод `.encode()` у строки.

Декодировать – _восстановить_ строку из цепочки байт. Вызвать метод `.decode()` у объекта `str` или `bytes` (версии Питона 2 и 3 соответственно).
...
_Диапазоны_ – неизменяемые последовательности чисел, которые задаются началом, концом и шагом.  
...
Последовательности одинаковых типов можно _сравнивать_. 
...
Объект называется _хешируемым_, если он имеет хеш-значение (целое число), которое никогда не изменяется на протяжении его жизненного цикла и возвращается методом `__hash__()`.
...
Хешируемый объект может сравниваться с другими объектами (реализует метод `__eq__()`). 
Равные хешируемые объекты должны иметь равные хеш-значения.  

- Все стандартные _неизменяемые_ объекты хешируемые. 
- Все стандартные _изменяемые_ объекты не хешируемые.

### Что такое множество

_Множество_ – это неупорядоченная коллекция хешируемых объектов, которые не повторяются.  
...
Обычно используются для проверки элемента на _вхождение_ в множество и _удаление повторений_ элементов и выполнения таких операций, как объединение, пересечение, разница и симметрическая разница.
...
_Отображение_ (mapping) – это объект-контейнер, который поддерживает произвольный доступ к элементам по ключам и описывает все методы, описанные в абстрактном базовом классе `collections.Mapping` (`get()`, `items()`, `keys()`, `values()`) или `collections.MutableMapping` (`clear()`, `get()`, `items()`, `keys()`, `pop()`, `popitem()`, `setdefault()`, `update()`, `values()`).
...
![Рис. Пирамида тестирования](https://github.com/AlexRogalskiy/python_interview_questions/blob/adcaa9e332900964367c8cbd31cf3d9419b7167f/attachments/eb2d6637.png)
...
![Рис. Интеграционное тестирование против функционального тестирования](https://github.com/AlexRogalskiy/python_interview_questions/blob/adcaa9e332900964367c8cbd31cf3d9419b7167f/attachments/aed78f00.png)
