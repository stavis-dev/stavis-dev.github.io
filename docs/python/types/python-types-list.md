...
Добавление элемента в список осуществляется с помощью метода [append()](#list-append}).

Для удаления элемента из списка, в случае, если вы знаете его значение, используйте метод [remove(x)](#list-remove), при этом будет удалена первая ссылка на данный элемент.

Если необходимо удалить элемент по его индексу, воспользуйтесь командой [del](#list-del) имя_списка[индекс].
...
Сортирует элементы в **текущем** списке по возрастанию. 

> Для создания нового отсортированного списка исользуют функцию [sorted](../functions/python-sorted.md).  
...
[List Comprehensions](./x_comprehensions.md#list-comprehensions) чаще всего на русский язык переводят как  абстракция списков или списковое включение, является частью синтаксиса языка, которая предоставляет простой способ построения списков. Тема обязательна к изучению.
